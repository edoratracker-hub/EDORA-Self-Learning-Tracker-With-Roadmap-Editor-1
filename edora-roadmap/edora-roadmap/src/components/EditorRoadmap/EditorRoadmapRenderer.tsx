...
import { CustomReactFlow as Renderer } from './CustomReactFlow';
...
type RoadmapNodeDetails = {
  nodeId: string;
  nodeType: string;
  targetGroup: SVGElement | HTMLElement;
  title?: string;
};

function getNodeDetails(element: Element): RoadmapNodeDetails | null {
  const targetGroup = (element?.closest('.roadmap-node') || element?.closest('g')) as HTMLElement | SVGElement | null;
  if (!targetGroup) return null;

  const nodeId = targetGroup.dataset?.nodeId;
  const nodeType = targetGroup.dataset?.type;
  const title = (targetGroup as HTMLElement).dataset?.title || targetGroup.dataset?.title;
...
export function EditorRoadmapRenderer(props: RoadmapRendererProps) {
  const { resourceId, nodes = [], edges = [], dimensions } = props;
  ...
  return (
    <div 
      ref={(node) => {
        // @ts-ignore - roadmapRef is RefObject but we are fine manually assigning
        roadmapRef.current = node;
        if (node) {
          node.setAttribute('data-renderer', 'editor');
