...
export function TopicDetail(props: TopicDetailProps) {
  ...
  const setMessages = (_messages: any[]) => { };
  const status = 'error';
  ...
  const tokenUsage = { used: 0, limit: Number.MAX_SAFE_INTEGER };
  const isTokenUsageLoading = false;
  ...
  useLoadTopic(({ topicId, resourceType, resourceId, isCustomResource }) => {
    ...
    if (isCustomResource) {
      topicUrl = `${import.meta.env.PUBLIC_API_URL
        }/v1-get-node-content/${resourceId}/${topicId}${secret ? `?secret=${secret}` : ''
        }`;
    }
  ...
  return (
    <div className={cn('relative z-92', wrapperClassName)}>
      <div
        ...
        tabIndex={0}
        className={cn(
          'fixed top-0 right-0 z-40 flex h-screen w-full flex-col overflow-y-auto bg-gray-800 p-4 focus:outline-0 sm:max-w-[600px] sm:p-6',
          bodyClassName,
        )}
      >
        {showUpgradeModal && (
            ...
            <div className="flex-1">
              <div className="flex justify-between">
                ...
                >
                  <button
                    ...
                    id="close-topic"
                    className={cn(
                      'flex items-center gap-1.5 rounded-lg bg-gray-700 px-1.5 py-1 text-xs text-white hover:bg-gray-600',
                      closeButtonClassName,
                    )}
                    onClick={handleClose}
                  >
              ...
              {activeTab === 'content' && (
                <>
                  {hasContent ? (
                    <>
                      <div className="prose prose-invert prose-quoteless prose-h1:mb-2.5 prose-h1:mt-7 prose-h1:text-balance prose-h2:mb-3 prose-h2:mt-0 prose-h3:mb-[5px] prose-h3:mt-[10px] prose-p:mb-2 prose-p:mt-0 prose-blockquote:font-normal prose-blockquote:not-italic prose-blockquote:text-gray-300 prose-li:m-0 prose-li:mb-0.5 prose-a:text-blue-400">
                    ...
                    <>
                      ...
                      {canSubmitContribution && (
                        <div className="mx-auto flex h-[calc(100%-38px)] max-w-[400px] flex-col items-center justify-center text-center">
                          <HeartHandshake className="mb-2 h-16 w-16 text-gray-500" />
                  ...
                  {resourceId === 'ai-data-scientist' &&
                    hasDataCampResources && (
                      <div className="mt-5 rounded-md bg-yellow-900/30 px-4 py-3 text-sm text-gray-200">
        ...
        {!isContributing && !isLoading && error && (
          <>
            <button
              type="button"
              id="close-topic"
              className="absolute top-2.5 right-2.5 inline-flex items-center rounded-lg bg-transparent p-1.5 text-sm text-gray-400 hover:bg-gray-700 hover:text-white"
              onClick={() => {
                handleClose();
              }}
            >
            ...
            <div className="flex h-full flex-col items-center justify-center">
              ...
              <p className="mt-2 text-lg font-medium text-red-400">{error}</p>
            </div>
          </>
        )}
      </div>
