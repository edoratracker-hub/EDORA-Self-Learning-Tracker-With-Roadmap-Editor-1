...
export const DailyToDoList = () => {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('mentor-daily-todo-status')
    ...
    if (saved) {
      try {
        ...
        if (date === today) {
          ...
          // New day, reset!
          localStorage.removeItem('mentor-daily-todo-status')
        }
      } catch (e) {
  ...
  useEffect(() => {
    if (isLoaded && lastDate) {
      localStorage.setItem('mentor-daily-todo-status', JSON.stringify({
        date: lastDate,
