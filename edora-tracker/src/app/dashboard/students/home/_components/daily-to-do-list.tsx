...
export const DailyToDoList = () => {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('student-daily-todo-status')
    ...
    if (saved) {
      try {
        ...
        if (date === today) {
          ...
          // New day, reset!
          localStorage.removeItem('student-daily-todo-status')
        }
      } catch (e) {
  ...
  useEffect(() => {
    if (isLoaded && lastDate) {
      localStorage.setItem('student-daily-todo-status', JSON.stringify({
        date: lastDate,
  ...
  const toggleTask = async (task: DailyTask) => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(`Challenge complete! +${task.points} points earned!`, {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="mt-1 font-medium text-blue-600/80 dark:text-blue-400">
              Increase your score with daily habits
            </CardDescription>
          </div>
          <div className={cn(
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5">
                     <TrendingUp className={cn("size-3", isCompleted ? "text-emerald-500/50" : "text-blue-500/80")} />
                     <span className={cn("text-[10px] font-bold uppercase tracking-wider", isCompleted ? "text-emerald-600/60" : "text-blue-600/80")}>
                       +{task.points} Pts
                     </span>
                  </div>
                </div>
