...
const buildContributionGrid = (
  ...
  while (currentDate <= yearEnd) {
    ...
    if (currentDate < yearStart || currentDate > yearEnd) {
      ...
      // Future days in the year — show empty square
      currentWeek.push({
...
export function ActivityHeatmap() {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (result.accountCreatedAt) {
            setAccountCreatedAt(new Date(result.accountCreatedAt));
          }
        }
      } catch {
  ...
  const contributionData = useMemo(
    () => buildContributionGrid(activityMap, accountCreatedAt ?? new Date()),
    [activityMap, accountCreatedAt],
  );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {totalContributions} activities in {new Date().getFullYear()}
        </CardDescription>
      </CardHeader>
      <CardContent>
        {loading ? (
            ...
            <div className="w-full ">
              ...
              <div className="flex justify-between mb-2 pl-8">
                {monthLabels.map((label, index) => {
                  const now = new Date();
                  const isCurrentMonth = label.month === MONTHS[now.getMonth()];
                  return (
                    <div
                      key={index}
                      className={`text-xs ${isCurrentMonth ? "text-primary font-semibold" : "text-muted-foreground"}`}
                      style={{
                        marginLeft:
                          index === 0
                            ? 0
                            : `${(label.weekIndex - (monthLabels[index - 1]?.weekIndex || 0)) * 12}px`,
                      }}
                    >
                      {label.month}
                    </div>
              ...
              <div className="flex gap-[2px]">
                ...
                <div className="flex flex-col gap-[2px] shrink-0">
                  {DAYS_OF_WEEK.map((day, index) => (
                    <div
                      key={day}
                      className="text-xs text-muted-foreground h-[10px] flex items-center leading-none"
                      style={{
                        ...
                        {week.days.map((day, dayIndex) => (
                          <Tooltip key={`${weekIndex}-${dayIndex}`}>
                            <TooltipTrigger asChild>
                              <div
                                className={`w-full aspect-square rounded-sm border border-background transition-colors cursor-pointer ${day
              ...
              <div className="flex items-center gap-2 mt-4 text-xs text-muted-foreground">
                ...
                <div className="flex gap-[2px]">
                  {[0, 1, 2, 3, 4].map((level) => (
                    <div
                      key={level}
                      className={`w-[10px] h-[10px] rounded-sm border border-background ${getLevelColor(level)}`}
                    />
                  ))}
                </div>
                <span>More</span>
