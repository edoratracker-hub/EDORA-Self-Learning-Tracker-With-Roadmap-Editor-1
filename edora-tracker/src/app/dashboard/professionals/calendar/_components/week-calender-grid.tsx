                ...
                {weekDays.map((date, dayIndex) => (
                  ...
                  >
                    ...
                    {timeSlots.map((hour) => (
                      <div
                        key={`${dayIndex}-${hour}`}
                        className="h-16 border-b hover:bg-neutral-900/50 transition-colors cursor-pointer"
                        onDoubleClick={() => {
                          const clickedDate = new Date(date);
                          clickedDate.setHours(hour, 0, 0, 0);
                          (onEventClick as any)?.(clickedDate, true); // Overloading for now, or use a separate prop
                        }}
                      />
                    ))}
