...
export const JobVacancyForm = ({ children }: JobVacancyFormProps) => {
  ...
  const onSubmit = async (data: JobOpportunityFormData) => {
    ...
    try {
      ...
      const result = await createJobOpportunity(formData);

      if (result.success) {
        handleReset();
        setOpen(false);
      } else {
        setError(result.error || "Failed to create job opportunity");
      }
    } catch (err) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        {children || <Button>Post a Job</Button>}
      </SheetTrigger>
      <SheetContent
        ...
        <SheetHeader className="flex flex-row items-start justify-between space-y-0 bg-muted px-4 py-4 text-left shadow-sm z-10">
          <div className="space-y-1">
            ...
            <SheetDescription className="text-xs">
              Enter the details below. Required fields are marked with an
              asterisk.
            </SheetDescription>
          </div>
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          ...
                          >
                            ...
                            {watch("applicationDeadline") ? format(watch("applicationDeadline") as Date, "PPP") : <span>Pick a date</span>}
                          </Button>
                        </PopoverTrigger>
